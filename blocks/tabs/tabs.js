...
export default async function decorate(block) {
  const presentTabContents = [...block.querySelectorAll(':scope > div.contents-wrapper > div')];
  ...
  if (presentTabContents && presentTabContents.length > 0) {
    ...
    const promises = presentTabContents.map(async (contents) => {
      ...
      //Slider Start
      try {
        const tempData = contents.querySelector(".cards");
        tempData.classList.add('swiper');
        const swiperWrapper = document.createElement("div");
        swiperWrapper.classList.add('swiper-wrapper');

        const childRelative = contents.querySelectorAll(".cards > div");
        Array.from(childRelative).forEach((row) => {
          row.classList.add('swiper-slide');
          swiperWrapper.appendChild(row);
        });

        tempData.append(swiperWrapper);

        const RightArrow = document.createElement("div")
        RightArrow.classList.add("swiper-button-next");
        tempData.appendChild(RightArrow);
